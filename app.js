...
async function save_message_to_db(ad_id, conv_data, ad_details) {
  try {
    ...
    const qry2 =
      "replace INTO conversations (ad_id, conv_data, ad_details) VALUES (?, ?, ?)";
    await queryAsync(qry2, [
      ad_id,
      JSON.stringify(conv_data_arr),
      JSON.stringify(ad_details),
    ]);
  ...
  try {
    ...
    if (rslt.length > 0) {
      for (var i = 0; i < rslt.length; i++) {
        conv_data_arr.push({
          msg: rslt[i].conv_message,
          dt: rslt[i].creation_date,
          sender: rslt[i].sender,
...
async function insert_chat_to_db(
  ad_id,
  sender,
  dealer_name,
  conv_message,
...
function getReadableDate(timestamp) {
  ...
  const humanReadableDate = `${year}-${month.toString().padStart(2, "0")}-${day
    .toString()
...
async function fetchKijijiAutoData(SearchKey) {
  ...
  try {
    const response = await axios.get(
      "https://www.kijijiautos.ca/consumer/place-suggestions?type=suggestions&components=country:ca&input=" +
      SearchKey +
      "&sessionToken=e6212e03-15be-42ff-8856-4fdea7575b8d",
      {
        headers: headers,
      }
    );
...
async function processPageActions(req) {
  ...
  try {
    ...
    if (req.body.logout != undefined && filters.isLogedin == true) {
      ...
      await myBrowser.page.waitForSelector(
    ...
    if (filters.isLoginNeeded == undefined && req.body.reset == undefined) {
      try {
        await page.waitForSelector('[data-testid="navigation-menu-login"]', {
          timeout: 1500,
        });
      } catch (err) {
        console.log(err);
      ...
      const isloggedin = await page.evaluate(() => {
        var els = document.querySelectorAll(
          '[data-testid="navigation-menu-login"]'
        );
        if (els.length > 0) {
          return els[0].innerText != "Sign in";
        }
        var els2 = document.querySelectorAll(
          '[data-testid="buttons-logged-in-navigation-button"]'
        );
      ...
      await save_option_to_db(login_db_key, {
    ...
    if (req.body.back_to_search_results == undefined) {
      ...
      if (req.body.start_conversation != undefined) {
        ...
        if (filters.isLogedin == undefined) {
          ...
          filters.messageType = filters.isLoginNeeded
            ? "singleText"
            : "fullForm";
        }
      }
      ...
      if (req.body.send_message != undefined) {
        ...
        await myBrowser.page.evaluate(
          `var el = document.querySelectorAll('[data-testid="MessageButton"]');if(el.length>0){el[0].click()}`
        );
        ...
        await myBrowser.page.type("#messageInquery", req.body.conv_msg, {
          delay: 1,
          clear: true,
        });
        ...
        if (filters.messageType == "fullForm") {
          ...
          if (await isExist(myBrowser.page, "#nameInquery")) {
            ...
            await myBrowser.page.type("#nameInquery", req.body.fullName, {
              delay: 5,
              clear: true,
            });
          ...
          if (await isExist(myBrowser.page, "#emailInquery")) {
            ...
            await myBrowser.page.type("#emailInquery", req.body.emailAddress, {
              delay: 5,
              clear: true,
            });
          ...
          if (await isExist(myBrowser.page, "#phoneInquery")) {
            ...
            await myBrowser.page.type("#phoneInquery", req.body.phone, {
              delay: 5,
              clear: true,
            });
          ...
          await myBrowser.page.evaluate(
            `var el = document.querySelector('label[for="financing-ocf"] input');if(el!=undefined && el.checked){el.parentElement.click();}`
          );
          if (req.body.financing != undefined) {
            await myBrowser.page.evaluate(
              `var el = document.querySelector('label[for="financing-ocf"]');if(el != undefined){el.click();}`
            );
            filters.conversation_data.financing = req.body.financing;
          }
          filters.conversation_data.testDrive = "";
          await myBrowser.page.evaluate(
            `var el = document.querySelector('label[for="testDrive-ocf"] input');if(el!=undefined && el.checked){el.parentElement.click();}`
          );
          if (req.body.testDrive != undefined) {
            await myBrowser.page.evaluate(
              `var el = document.querySelector('label[for="testDrive-ocf"]');if(el != undefined){el.click();}`
            );
            filters.conversation_data.testDrive = req.body.testDrive;
          }
          filters.conversation_data.trading = "";
          await myBrowser.page.evaluate(
            `var el = document.querySelector('label[for="modalTradeIn-ocf"] input');if(el!=undefined && el.checked){el.parentElement.click();}`
          );
          if (req.body.trading != undefined) {
            await myBrowser.page.evaluate(
              `var el = document.querySelector('label[for="modalTradeIn-ocf"]');if(el != undefined){el.click();}`
            );
        ...
        filters.sending_msg_errors = await myBrowser.page.evaluate(() => {
          ...
          var btn = document.querySelector(
            '[data-testid="OpenContactFormSendButton"]'
          );
          if (btn == undefined || btn == null) {
            btn = document.querySelector(
              '[data-testid="SendContactFormInfoButton"]'
            );
          }
          if (btn.disabled) {
            ...
            return msg == ""
              ? "Error: Please make sure all input fields are correct."
              : msg;
          }
          btn.click();
          return "";
        ...
        if (
          ...
          filters.send_feedback.indexOf("Message was sent") != -1
        ) {
          //get ad id
          ...
          await save_message_to_db(
            ad_id,
            filters.conversation_data,
            filters.start_conversation
          );
        }
      }
    } else {
    ...
    if (filters.results != undefined && req.body.reset == undefined) {
      console.log(filters);
    } else if (filters.isLogedin == true) {
      //else if there is no search done yet and user is logged in
      ...
      try {
        await page.waitForSelector('[data-testid="AdvancedSearchButton"]');
        await myBrowser.page
          .evaluate(`var el = document.querySelector('[data-testid="AdvancedSearchButton"]');
                if(typeof el != "undefined" && el != null){
                    el.click();
                }`);
        await page.waitForSelector('[data-testid="DetailSearchModalForm"]');
        await myBrowser.page
          .evaluate(`var els = document.querySelectorAll('[data-testid="DetailSearchModalForm"] button span');
        ...
        if (
          url != main_page_url &&
          req.body.conv_msg == undefined &&
          req.body.start_conversation == undefined
        ) {
          await page.goto(main_page_url);
          await page.waitForTimeout(1000);
          await page.waitForSelector('[data-testid="AdvancedSearchButton"]');
      ...
      if (req.body.filter_make) {
        //if filter_make form is submitted
        ...
        for (let i = 0; i < filters.make.length; i++) {
          script +=
            `document.querySelector('#MAKE_MODEL [name="` +
            filters.make[i] +
            `"]').parentElement.parentElement.children[1].click();`;
        }
        ...
        filters.model_data = await myBrowser.page.evaluate(() => {
          var els = document.querySelectorAll(
            '#MAKE_MODEL [data-testid="ModelFilterDesktopMenu"] li input'
          );
          var arr = [];
          for (var i = 0; i < els.length; i++) {
        ...
        await myBrowser.page.evaluate(() => {
          try {
            document
              .querySelector('#MAKE_MODEL [data-testid="ApplyButton"]')
              .click();
            document
              .querySelector('#MAKE_MODEL [data-testid="ApplyButton"]')
      ...
      if (req.body.reset != undefined) {
        //------ clear location
        await myBrowser.page.waitForSelector(
          'form[data-testid="DetailSearchModalForm"] button[data-testid="LocationLabelLink"]'
        );
        ...
        await myBrowser.page.evaluate(() => {
          ...
          els = document.querySelectorAll(
            'button[data-testid="LocationModalSubmitButton"]'
          );
          if (els.length > 0) {
            els[0].click();
          }
      ...
      if (req.body.filter_model) {
        //if filter_model form is submitted
        filters.model = [];
        ...
        for (let i = 0; i < filters.model.length; i++) {
          script +=
            `document.querySelector('#MAKE_MODEL  [data-testid="ModelFilterDesktopMenu"] [name="` +
            filters.model[i] +
            `"]').parentElement.parentElement.children[1].click();`;
        }
        ...
        filters.trim_data = await myBrowser.page.evaluate(() => {
          var els = document.querySelectorAll(
            '[data-testid="TrimFilterDesktopMenu"] li input'
          );
          var arr = [];
          for (var i = 0; i < els.length; i++) {
            arr.push(els[i].getAttribute("name"));
        ...
        await myBrowser.page.evaluate(() => {
          try {
            document.querySelector("#MAKE_MODEL #model").click();
            document
              .querySelector(
                '#MAKE_MODEL [data-testid="ModelFilterDesktopMenu"] button span'
              )
              .click();
            document
              .querySelector(
                '#MAKE_MODEL  [data-testid="ModelFilterDesktopMenu"] [data-testid="ApplyButton"]'
              )
              .parentElement.children[0].click();
            document
              .querySelector(
      ...
      // ---------------- trim
      if (req.body.filter_trim) {
        //if trim_data form is submitted
        filters.trim = [];
        ...
        for (let i = 0; i < filters.trim.length; i++) {
          script +=
            `document.querySelector('#MAKE_MODEL [data-testid="TrimFilterDesktopMenu"] [name="` +
            filters.trim[i] +
            `"]').parentElement.parentElement.children[1].click();`;
        }
        ...
        await myBrowser.page.evaluate(() => {
          try {
            document.querySelector("#trim").click();
            document
              .querySelector(
                '#MAKE_MODEL [data-testid="TrimFilterDesktopMenu"] button span'
              )
              .click();
            document
              .querySelector(
                '#MAKE_MODEL  [data-testid="TrimFilterDesktopMenu"] [data-testid="ApplyButton"]'
              )
              .parentElement.children[0].click();
            document
              .querySelector(
      ...
      if (
        filters.year_from_data == undefined ||
        filters.year_from_data.length == 0
      ) {
      ...
      if (
        filters.year_to_data == undefined ||
        filters.year_to_data.length == 0
      ) {
      ...
      if (req.body.year_from_data) {
        //if year_from_data form is submitted
        ...
        let script =
          `document.querySelector('#CONSTRUCTION_YEAR-from').click();
      ...
      if (req.body.year_to_data) {
        //if year_to_data form is submitted
        filters.year_to = "Max";
        ...
        let script =
          `document.querySelector('#CONSTRUCTION_YEAR-to').click();
      ...
      if (req.body.price_from) {
        //if price_from form is submitted
        filters.price_from = req.body.price_from;
        await clearValue(myBrowser.page, "#PRICE-from");
        await myBrowser.page.type("#PRICE-from", filters.price_from, {
          delay: 20,
          clear: true,
        });
        await myBrowser.page.waitForTimeout(small_wait);
      ...
      if (req.body.price_to) {
        //if price_to form is submitted
        filters.price_to = req.body.price_to;
        await clearValue(myBrowser.page, "#PRICE-to");
        await myBrowser.page.type("#PRICE-to", filters.price_to, {
          delay: 20,
          clear: true,
        });
      ...
      //---------------- transition
      if (req.body.tran_manual) {
        //if tran_manual form is submitted
        filters.tran_manual = req.body.tran_manual == "0" ? "0" : "1";

        await myBrowser.page.evaluate(
          `if(document.querySelector("#tr-manual").checked != ` +
          (filters.tran_manual == "1") +
          `){
      ...
      if (req.body.tran_automatic) {
        //if tran_automatic form is submitted
        filters.tran_automatic = req.body.tran_automatic == "0" ? "0" : "1";
        await myBrowser.page.evaluate(
          `if(document.querySelector("#tr-automatic").checked != ` +
          (filters.tran_automatic == "1") +
          `){
      ...
      //--------------- kilometres
      if (req.body.Kilometres_from) {
        //if Kilometres_from form is submitted
        filters.Kilometres_from = req.body.Kilometres_from;
        await clearValue(myBrowser.page, "#MILEAGE-from");
        await myBrowser.page.type("#MILEAGE-from", filters.Kilometres_from, {
          delay: 20,
          clear: true,
        });
      ...
      if (req.body.Kilometres_to) {
        //if Kilometres_to form is submitted
        filters.Kilometres_to = req.body.Kilometres_to;
        await clearValue(myBrowser.page, "#MILEAGE-to");
        await myBrowser.page.type("#MILEAGE-to", filters.Kilometres_to, {
          delay: 20,
          clear: true,
        });
    ...
    for(var i=0;i<els.length;i++){
      ...
      //---------------- Condition
      if (req.body.cond_used) {
        //if cond_used form is submitted
        filters.cond_used = req.body.cond_used == "0" ? "0" : "1";

        await myBrowser.page.evaluate(
          `if(document.querySelector("#con-used").checked != ` +
          (filters.cond_used == "1") +
          `){
      ...
      if (req.body.cond_new) {
        //if cond_new form is submitted
        filters.cond_new = req.body.cond_new == "0" ? "0" : "1";
        await myBrowser.page.evaluate(
          `if(document.querySelector("#con-new").checked != ` +
          (filters.cond_new == "1") +
          `){
      ...
      if (req.body.cond_cer_pre_owned) {
        //if cond_cer_pre_owned form is submitted
        filters.cond_cer_pre_owned =
          req.body.cond_cer_pre_owned == "0" ? "0" : "1";
        await myBrowser.page.evaluate(
          `if(document.querySelector("#con-certified_pre_owned").checked != ` +
          (filters.cond_cer_pre_owned == "1") +
          `){
      ...
      if (req.body.fuel_gas) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#ft-gas").checked != ` +
          (filters.fuel_gas == "1") +
          `){
      ...
      if (req.body.fuel_diesel) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#ft-diesel").checked != ` +
          (filters.fuel_diesel == "1") +
          `){
      ...
      if (req.body.fuel_electric) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#ft-electric").checked != ` +
          (filters.fuel_electric == "1") +
          `){
      ...
      if (req.body.fuel_hybrid) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#ft-hybrid").checked != ` +
          (filters.fuel_hybrid == "1") +
          `){
      ...
      if (req.body.fuel_other) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#ft-other").checked != ` +
          (filters.fuel_other == "1") +
          `){
      ...
      if (req.body.drv_four_wheel) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#dt-four_wheel").checked != ` +
          (filters.drv_four_wheel == "1") +
          `){
      ...
      if (req.body.drv_all_wheel) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#dt-awd").checked != ` +
          (filters.drv_all_wheel == "1") +
          `){
      ...
      if (req.body.drv_front_wheel) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#dt-fwd").checked != ` +
          (filters.drv_front_wheel == "1") +
          `){
      ...
      if (req.body.drv_rear_wheel) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#dt-rwd").checked != ` +
          (filters.drv_rear_wheel == "1") +
          `){
      ...
      if (req.body.drv_other) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#dt-other").checked != ` +
          (filters.drv_other == "1") +
          `){
      ...
      if (req.body.slr_dealer) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#st-dealer").checked != ` +
          (filters.slr_dealer == "1") +
          `){
      ...
      if (req.body.slr_private) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#st-private_seller").checked != ` +
          (filters.slr_private == "1") +
          `){
    ...
    for(var i=0;i<els.length;i++){
      ...
      //---------------------- Number of cylinders
      if (req.body.cylinders_from) {
        //if cylinders_from form is submitted
        filters.cylinders_from = req.body.cylinders_from;
        const script =
          `document.querySelector('#CYLINDERS-from').click();
    var els = document.querySelectorAll('[data-testid="CYLINDERS-RangeDropdown-minDesktopMenu"] li');
    for(var i=0;i<els.length;i++){
        if(els[i].innerText.indexOf("` +
          filters.cylinders_from +
          `") == 0){
            els[i].click();
        }
    }`;
    ...
    for(var i=0;i<els.length;i++){
      ...
      //---------------------- Power
      if (req.body.power_from) {
        //if power_from form is submitted
        filters.power_from = req.body.power_from;
        const script =
          `document.querySelector('#POWER_IN_KW-from').click();
    ...
    for(var i=0;i<els.length;i++){
      ...
      if (req.body.power_to) {
        //if power_to form is submitted
        filters.power_to = req.body.power_to;
        const script =
          `document.querySelector('#POWER_IN_KW-to').click();
    ...
    for(var i=0;i<els.length;i++){
      ...
      //---------------------- Seat count
      if (req.body.seat_from) {
        //if seat_from form is submitted
        filters.seat_from = req.body.seat_from;
        const script =
          `document.querySelector('#SEATS-from').click();
    ...
    for(var i=0;i<els.length;i++){
      ...
      if (req.body.seat_to) {
        //if seat_to form is submitted
        filters.seat_to = req.body.seat_to;
        const script =
          `document.querySelector('#SEATS-to').click();
    ...
    for(var i=0;i<els.length;i++){
      ...
      if (req.body.doors_two_or_three) {
        filters.doors_two_or_three =
          req.body.doors_two_or_three == "0" ? "0" : "1";

        await myBrowser.page.evaluate(
          `if(document.querySelector("#doors-TWO_OR_THREE").checked != ` +
          (filters.doors_two_or_three == "1") +
          `){
      ...
      if (req.body.doors_four_or_five) {
        filters.doors_four_or_five =
          req.body.doors_four_or_five == "0" ? "0" : "1";

        await myBrowser.page.evaluate(
          `if(document.querySelector("#doors-FOUR_OR_FIVE").checked != ` +
          (filters.doors_four_or_five == "1") +
          `){
      ...
      if (req.body.with_video) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#MEDIA-video").checked != ` +
      ...
      if (req.body.filter_location) {
        ...
        await myBrowser.page.type("#LocationAutosuggest", filters.location, {
          delay: 20,
          clear: true,
        });
        //choose location
        await myBrowser.page.waitForSelector(
          'ul[data-testid="Autosuggest-Menu"] li'
        );
        await myBrowser.page.evaluate(
          `document.querySelector('ul[data-testid="Autosuggest-Menu"] li').click();`
        );
        //choose radius
        await myBrowser.page.evaluate(
          `
        ...
        await myBrowser.page.evaluate(
          `
                    if(document.querySelector('#limitedToProvince').checked != ` +
          isSelectRadiusLimit +
          `){
        ...
        await myBrowser.page.evaluate(
          `document.querySelector('button[data-testid="LocationModalSubmitButton"]').click();`
        );
      }
      if (req.body.with_photo) {
        ...
        await myBrowser.page.evaluate(
          `if(document.querySelector("#MEDIA-image" ).checked != ` +
          (filters.with_photo == "1") +
          `){
      ...
      filters.results_button = await myBrowser.page.evaluate(
        `var el = document.querySelector('[data-testid="AllFiltersResultButton"]');if(el != undefined){el.innerText}else{"Apply"}`
      );
      ...
      if (req.body.show_results == "show") {
        ...
        await myBrowser.page.evaluate(
          `document.querySelector('[data-testid="AllFiltersResultButton"]').click();`
        );
              ...
              for (let j = 0; j < listings.items.length; j++) {
                ...
                if (item.badges.length > 0) {
                  for (let j = 0; j < item.badges.length; j++) {
                    if (
                      item.badges[j].label &&
                      item.badges[j].label == "Free CARFAX Report"
                    ) {
                      carfax_status = "free";
                      break;
                    } else {
                ...
                if (
                  filters.carfax != "all" &&
                  ((filters.carfax == "free" && carfax_status != "free") ||
                    (filters.carfax == "request" && carfax_status != "request"))
                )
                  continue;
...
async function get_results(page) {
  ...
  page.evaluate(
    "document.scrollingElement.scrollTop=10;var iv1 = setInterval(function(){document.scrollingElement.scrollTop += 300; },10);var iv2 = setInterval(function(){var moreBtn = document.getElementsByClassName('infinite-scroller__show-more-button')[0];moreBtn.click();},1500);"
  );
  ...
  let rslt = await page.evaluate(() => {
    ...
    var elms = document.querySelectorAll(
      "[data-testid='SearchResultList']  article[data-testid='SearchResultListItem']"
    );
    for (let i = 0; i < elms.length; i++) {
      try {
        ...
        try {
          ad_id = elms[i]
            .querySelector("[data-test-ad-id]")
            .getAttribute("data-test-ad-id")
            .trim();
        } catch { }
        ...
        try {
          price = elms[i]
            .querySelector("[data-testid='searchResultItemPrice']")
            .innerText.trim();
        } catch { }
        ...
        try {
          loc = elms[i]
            ...
            .parentElement.querySelector(
              "[data-testid='VehicleListItemAttributeValue']"
            )
            .innerText.trim();
        } catch { }
        ...
        try {
          description_html = elms[i].querySelector("[aria-label='Location']")
            .parentElement.parentElement.innerHTML;
        } catch { }

        let dealer_updates = "";
        try {
          dealer_updates = elms[i]
            .querySelector("div.covid-response")
            .innerText.trim();
        } catch { }
        ...
        try {
          imageSrc = elms[i]
            .querySelector("[data-testid='VehicleListItem-figure'] img")
            .getAttribute("src")
            .trim();
        } catch { }
...
app.get(
  ...
  async (req, res) => {
    ...
    const is_login_needed = await page.evaluate(
      () => {
        return (
          document.querySelectorAll('[data-testid="NotLoggedInLoginButton"]')
            .length > 0
        );
      },
      { delay: 500 }
    ...
    try {
      ...
      await myBrowser.page.waitForSelector(
        "[data-testid='InboxContent'],[data-testid='EmptyInboxButton']"
      );
    ...
    if (conv_arr.length == 0) {
      //////////////////////////////// first try /////////////////////////////
      if (url.indexOf("kijijiautos.ca/inbox") == -1) {
        await myBrowser.page.goto("https://www.kijijiautos.ca/inbox", {
  ...
  async (req, res) => {
    ...
    if (req.body.action == undefined || req.body.action != "updateMessages") {
      if (
        ...
        conv_data.length > 0 &&
        myBrowser.page != undefined &&
        myBrowser.page !== false &&
        url.indexOf("kijijiautos.ca/inbox") != -1
      ) {
        ...
        await myBrowser.page.evaluate(
          (sender_name, sender_title) => {
            var els = document.querySelectorAll(
              '[data-testid="ChatResumeCard"]'
            );
            for (let i = 0; i < els.length; i++) {
              if (sender_title) {
                if (
                  els[i].innerText.indexOf(sender_name) != -1 &&
                  els[i].innerText.indexOf(sender_title) != -1
                ) {
                  els[i].click();
                }
              } else {
      ...
      if (req.body.sender_name && req.body.conv_txt) {
        ...
        await myBrowser.page.type(
          '[data-testid="MessageInputTextArea"]',
          req.body.conv_txt,
          { delay: 1, clear: true }
        );
        await myBrowser.page.evaluate(
          `document.querySelector('[data-testid="SendMessageButton"]').click();`
        );
      }
    }
    ...
    if (
      req.body.conv_id &&
      conv_data.length > 0 &&
      conv_data[conv_data.length - 1].conversations.length > 0
    ) {
    ...
    let current_chat_messages = await myBrowser.page.evaluate(() => {
      ...
      for (var i = 0; i < els.length; i++) {
        if (els[i].querySelectorAll("span").length > 1) {
          ...
          rslt_arr.push({
            txt: spans[0].innerText,
            time: spans[1].innerText,
            classes: classes,
          });
        }
      }
    ...
    if (current_chat_messages.length >= 0) {
      for (var i = 0; i < current_chat_messages.length; i++) {
        const msg_md5 = MD5(
          current_chat_messages[i].txt + "\r\n" + current_chat_messages[i].time
        );
        if (saved_messages_logger.indexOf(msg_md5) == -1) {
          if (
            current_chat_messages[i].classes.length > 0 &&
            current_chat_messages[i].classes.indexOf("h3--no") == -1
          ) {
            sender = "him";
          }
          await insert_chat_to_db(
            req.body.conv_id,
            sender,
            req.body.sender_name,
            current_chat_messages[i].txt,
...
app.post("/sendMsgs", async (req, res) => {
  if (!is_sending_started && req.body.urls && req.body.urls.length > 0) {
    ...
    res.json({
      sending: is_sending_started,
      sending_progress: sending_progress,
    });

    const urls = req.body.urls;
    for (let i = 0; i < urls.length; i++) {
      await myBrowser.page.goto(urls[i], {
      ...
      // await myBrowser.page.evaluate(`var el = document.querySelector('#messageInquery');if(el != undefined){el.innerText="";el.value='';}`);
      await clearValue(myBrowser.page, "#messageInquery");
      await myBrowser.page.type("#messageInquery", def_msg, {
        delay: 1,
        clear: true,
      });
      ...
      if (await isExist(myBrowser.page, "#nameInquery")) {
        await clearValue(myBrowser.page, "#nameInquery");
        await myBrowser.page.type("#nameInquery", fullName, {
          delay: 5,
          clear: true,
        });
      }
      ...
      if (await isExist(myBrowser.page, "#emailInquery")) {
        await clearValue(myBrowser.page, "#emailInquery");
        await myBrowser.page.type("#emailInquery", emailAddress, {
          delay: 5,
          clear: true,
        });
      }
      ...
      if (
        ...
        send_feedback.indexOf("Message was sent") != -1
      ) {
        //get ad id
        const url = urls[i];
        const idx = url.indexOf("/vip/") + 5;
...
app.get(
  ...
  async (req, res) => {
    ...
    res.render("queue_status", {
      counts: rslt,
      current_wait_time: db_wait_time,
      filters: filters,
    });
    await start_queue_processing();
  }
